...
@Component({
  selector: 'app-form-viewer-qout-confirm',
  templateUrl: './form-viewer-qout-confirm.component.html',
  styleUrls: ['./form-viewer-qout-confirm.component.scss']
})
export class RormViewerQoutConfirm implements OnInit, AfterViewInit{
  ...
  // =========================View Selected Quot==========================
    public async viewSelectedInvoice(Quataion: any){
debugger
    ...
    this.quatationPrimeID = ''
      ...
      this.custID = Quataion.quatcustId
      this.quatationPrimeID = Quataion.quatid;
      this.quatationisConfirmed = Quataion.quatisConfirmed,
      this.quatationJobNumber = Quataion.quatjobNumber
      this.quatationNotes = Quataion.quatnotes
      this.quatationPreparedBy = Quataion.quatpreparedBy
      this.selectedQuotProdId = Quataion.quatprodId
      this.quatationProdRefNu = Quataion.quatprodRefN
      this.quatationqDate  = moment(Quataion.quatquotDate).format('YYYY-MM-DD')

      this.quatationNumber = Quataion.quatquotNumber
      this.quatationTot = Quataion.quattotalAmount
      this.quatationvalidUntil = moment(Quataion.quatvalidUntil).format('MM-DD-YYYY')
      this.totalAmount = this.quatationTot
      this.quatationIsConfirmed = Quataion.isConfirmed,
      this.quatationIsRejected = Quataion.isRejected,
      this.rejectedOn = moment(Quataion.rejectedOn).format('YYYY-MM-DD')
      this.employeeNote = Quataion.employeeNote
      this.confiremdOn = moment(Quataion.confirmedOn).format('YYYY-MM-DD')
      this.empID = Quataion.empID
  ...
  async updateQuotConfirm(quatation:any){

    console.log(quatation)
    let reqFields = {
      "id": quatation.id,
      "quotID": quatation.quatid,
      "empID": quatation.empID,
      "status": 0,
      "isConfirmed": quatation.isConfirmed,
      "isRejected": quatation.isRejected,
    ...
    try {
      ...
      let updateRes = await this.extApi.UpdateQuotationConfirmEmployeeItem(reqFields)
